...
def detect():
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        conf_thres = 0.5
        iou_thres = 0.5
        classes = [0] # [0,1,2,3,5,7] # Filtro de clases
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(list(data_deque.keys())) > 0:
                    aforo_label = f'Total Number of People Detected: {max(list(data_deque.keys()))}'
                    x1, y1 = 30, 30
                    t_size = cv2.getTextSize(aforo_label, 0, 2/3, 1)[0]
                    cv2.rectangle(im0, (x1, y1-t_size[1]-3), (x1 + t_size[0], y1+3), [0,0,0], -1, cv2.LINE_AA)
                    cv2.putText(im0, aforo_label, (x1, y1 - 2), 0, 2/3, [225, 255, 255], 1, cv2.LINE_AA)

                    current_label = f'Current Number of People Detected: {len(list(data_deque.keys()))}'
                    x1, y1 = 30, 60
                    t_size = cv2.getTextSize(current_label, 0, 2/3, 1)[0]
                    cv2.rectangle(im0, (x1, y1-t_size[1]-3), (x1 + t_size[0], y1+3), [0,0,0], -1, cv2.LINE_AA)
                    cv2.putText(im0, current_label, (x1, y1 - 2), 0, 2/3, [225, 255, 255], 1, cv2.LINE_AA)
